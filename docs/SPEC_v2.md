...
smartRoute v2 is a **bounded coding-agent engine** for Codex. It is not a simple delegation tool — it is a controlled implementation loop where a worker model can read, modify, test, and repair within explicit boundaries, while Codex retains judgment, review, and commit authority.
...
_This spec defines smartRoute v2 as a managed worker architecture — not a multi-agent chat, but a bounded loop with a primary controller (Codex), a constrained executor (Worker), and a deterministic gate (Verifier)._
